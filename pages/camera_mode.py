...
# Load YOLO model
model_yolo = YOLO("yolov8m.pt")  # you can change to yolov8s.pt or yolov8m.pt
        ...
        while run:
            ...
            if descriptions:
                ...
                for label, text, image_pil in descriptions:
                    ...
                    unique_key = f"{label}_{uuid.uuid4().hex[:8]}"
                    if st.button(f"🤖 Ask Gemini about this", key=unique_key):
                        with st.spinner("Gemini is thinking..."):
                            response = model.generate_content(
                                f"Explain this object labeled '{label}' that contains the text: {text}"
                            )
                            st.successç(response.text)
